Create a single horizontal sprite strip for the Codex app digital pet `xiaomi` in the state `waiting`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Cute cartoon kitten digital pet, cheerful and expressive, can smile, look dejected, and act mischievous or playful. Compact chibi cat body, rounded head, tiny paws, bright friendly eyes..
