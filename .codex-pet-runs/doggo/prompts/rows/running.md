Create a single horizontal sprite strip for the Codex app digital pet `doggo` in the state `running`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Doggo is a small pixel-art-adjacent chibi dog with a round face, floppy ears, tiny paws, wagging tail, friendly expression, thick dark outline, warm tan and cream palette, no accessories..
