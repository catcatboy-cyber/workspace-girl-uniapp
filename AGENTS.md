...
## 字号 8 档（对齐 uni.scss 全局变量，禁止使用其他字号）
| SCSS 变量 | 字号 | 用途 |
|-----------|------|------|
| `$fs-display` | 50rpx | 分数展示、大数字 |
| `$fs-hero-title` | 42rpx | 页面主标题 |
| `$fs-kpi` | 40rpx | 关键指标数字 |
| `$fs-heading` | 38rpx | 卡片标题 section-title-v2、按钮 |
| `$fs-body-lg` | 34rpx | 正文、描述、说明、输入框 |
| `$fs-body` | 32rpx | 标签、副标题、辅助文案、图表标注 |
| `$fs-caption` | 24rpx | 脚注、微型说明、证据来源 |
| `$fs-micro` | 22rpx | 极小标注（倒计时、方向标签） |

## 字重（对齐 uni.scss 全局变量）
| SCSS 变量 | 字重 | 用途 |
|-----------|------|------|
| `$fw-hero` | 900 | 大标题、KPI、Hero 标题 |
| `$fw-heading` | 800 | 卡片标题、标签、按钮 |
| `$fw-label` | 700 | 输入标签、强调标签 |
| `$fw-body` | 400 | 正文、描述、说明、辅助文案 |
