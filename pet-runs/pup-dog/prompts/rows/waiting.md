Create a single horizontal sprite strip for the Codex app digital pet `pup` in the state `waiting`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: friendly small dog, floppy ears, warm tan and white coat, tiny tail, expressive dark eyes, compact chibi digital pet.
