...
async function load(){try{if(!resultId.value)throw new Error('缺少测试结果 ID');const history=await getArchetypeResults({kind:'crush_celebrity',resultId:resultId.value,limit:1});result.value=(history?.results||[]).find((item:any)=>item?._id===resultId.value);if(!result.value)throw new Error('结果不存在');const bank=await getArchetypeQuestionBank(FEATURE_CRUSH_CELEBRITY,result.value.contentVersion);if(!bank?.success)throw new Error(bank?.message||'题库读取失败');content.value=bank.bank.content}catch(error:any){errorMessage.value=error?.message||'读取失败'}finally{loading.value=false}}
function openPerson(key:string){uni.navigateTo({url:`/pages/crush-celebrity-person/crush-celebrity-person?personKey=${key}`})}
function retest(){uni.redirectTo({url:`/pages/crush-celebrity/crush-celebrity?mode=${result.value.mode}${result.value.caseId?`&caseId=${result.value.caseId}`:''}`})}
onLoad((options:any)=>{resultId.value=String(options?.id||'');load()})
onShow(()=>{themeVars.value=getThemeStyle();applyThemeChrome()})
onShareAppMessage(()=>({title:buildCelebrityShareCopy({mode:result.value?.mode,primary:primary.value,primarySimilarity:result.value?.similarities?.[result.value?.primaryPersonKey]||0,secondary:runnerUp.value,secondarySimilarity:result.value?.topFive?.[1]?.similarity||0,template:content.value?.resultCopy?.shareTemplate}),path:'/pages/crush-celebrity/crush-celebrity?mode=self'}))
</script>
