...
async function loadReport() { const sequence = ++loadSequence; loading.value = true; errorMessage.value = ''; try { const response = await getArchetypeReport(resultId.value); if (!response?.success || !response.report) throw new Error(response?.message || '读取结果失败'); if (sequence !== loadSequence) return; report.value = response.report } catch (error: any) { if (sequence === loadSequence) errorMessage.value = error?.message || '读取结果失败' } finally { if (sequence === loadSequence) loading.value = false } }
...
async function retest() {
  ...
  try {
    ...
    uni.redirectTo({ url: `/pages/dimension-character/dimension-character?${query.join('&')}` })
  } finally {
...
onShareAppMessage(() => !shareReady.value ? {} : ({ title: `${report.value?.mode === 'target' ? 'TA' : '我'}的次元角色风格报告`, path: appendReferralParams(`/pages/heart-persona-share/heart-persona-share?resultShareId=${encodeURIComponent(resultShareId.value)}`, 'heart_persona_result') }))
</script>
