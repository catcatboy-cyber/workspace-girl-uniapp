  ...
  <view class="page" :style="themeVars">
    ...
    <template v-else-if="report">
      ...
      <template v-else>
        ...
        <view class="card"><text class="card-title">前五角色</text><view v-for="(item,index) in report.topFive || []" :key="item.personKey" class="rank"><text>{{ index + 1 }} · {{ item.name }}</text><text>{{ item.similarity }}%</text></view></view>
...
async function loadReport() { loading.value = true; errorMessage.value = ''; try { const response = await getArchetypeReport(resultId.value); if (!response?.success || !response.report) throw new Error(response?.message || '读取结果失败'); report.value = response.report } catch (error: any) { errorMessage.value = error?.message || '读取结果失败' } finally { loading.value = false } }
function retest() { uni.redirectTo({ url: `/pages/dimension-character/dimension-character?mode=${report.value.mode}${report.value.caseId ? `&caseId=${report.value.caseId}` : ''}` }) }
...
onShareAppMessage(() => isReferralShareBlocked() ? {} : ({ title: `${report.value?.mode === 'target' ? 'TA' : '我'}的次元角色风格报告`, path: appendReferralParams('/pages/dimension-character/dimension-character?mode=self', 'dimension_character') }))
</script>
