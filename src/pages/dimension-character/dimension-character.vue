<template>
  <view class="page" :style="themeVars">
    <view class="hero">
      <text class="eyebrow">DIMENSION CHARACTER ATLAS</text>
      <text class="title">次元角色图鉴</text>
      <text class="subtitle">12 道行为题，看看{{ mode === 'target' ? ' TA' : '你' }}像哪位名著、影视或动漫角色</text>
    ...
    <view v-if="loading" class="card">正在加载 72 人角色图鉴...</view>
    ...
    <view v-else-if="phase === 'mode-select'" class="card">
      ...
      <button class="choice" @click="chooseMode('self')">测自己<text>我像哪位名著、影视或动漫角色</text></button>
      <button class="choice" @click="chooseMode('target')">测当前 Crush<text>看看 TA 的次元角色风格</text></button>
      <button class="secondary" @click="uni.navigateTo({ url: '/pages/dimension-character-history/dimension-character-history' })">查看历史记录</button>
    </view>

    <view v-else-if="phase === 'quiz' && currentQuestion" class="card">
      <ArchetypeQuizProgress :current="questionIndex + 1" :total="questions.length" label="角色匹配" />
    ...
    <view v-else class="card">正在计算五维画像和 72 人相似度...</view>
  </view>
</template>
...
async function initialize() {
  ...
  try {
    const access = await checkFeatureAccess(FEATURE_DIMENSION_CHARACTER)
    if (!access?.success || !access?.allowed) {
      accessDenied.value = true
      errorMessage.value = '当前套餐未开放次元角色图鉴。'
      return
    }
...
async function submit() {
  ...
  try {
    ...
    uni.redirectTo({ url: `/pages/dimension-character-result/dimension-character-result?id=${encodeURIComponent(response.result._id)}` })
  } catch (error: any) {
