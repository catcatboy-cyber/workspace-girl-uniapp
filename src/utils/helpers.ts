...
export type PetId = 'xiaomi' | 'doggo'
...
const sharedPetStates: Record<string, PetStateConfig> = {
...
export function normalizePetId(value: any): PetId {
  return value === 'doggo' ? 'doggo' : 'xiaomi'
}

export function getPetById(value: any): PetOption {
  const id = normalizePetId(value)
  return petOptions.find(pet => pet.id === id) || petOptions[0]
}

export function getSelectedPetId(): PetId {
  try {
...
export function setSelectedPetId(id: PetId) {
