...
async function ensureSubscriptionConfig(db) {
  try {
    ...
    if (data && data.length > 0) {
      ...
      set(existing, 'tokenExchangeRate', DEFAULT_SUBSCRIPTION_CONFIG.tokenExchangeRate)
      set(existing, 'featureEstTokens', { ...DEFAULT_SUBSCRIPTION_CONFIG.featureEstTokens })
      set(existing, 'welcomeTokens', DEFAULT_SUBSCRIPTION_CONFIG.welcomeTokens)
