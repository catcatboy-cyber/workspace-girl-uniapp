...
async function ensureSubscriptionConfig(db) {
  try {
    ...
    if (data && data.length > 0) {
      ...
      if (existing.tokenExchangeRate === undefined) { existing.tokenExchangeRate = DEFAULT_SUBSCRIPTION_CONFIG.tokenExchangeRate; needsUpdate = true }
      if (existing.featureEstTokens === undefined) { existing.featureEstTokens = { ...DEFAULT_SUBSCRIPTION_CONFIG.featureEstTokens }; needsUpdate = true }
      if (existing.welcomeTokens === undefined && existing.welcomeCalls !== undefined) { existing.welcomeTokens = existing.welcomeCalls * 500; needsUpdate = true }
      if (existing.welcomeTokens === undefined) { existing.welcomeTokens = DEFAULT_SUBSCRIPTION_CONFIG.welcomeTokens; needsUpdate = true }
      // 升级 plans 中的旧字段名
      for (const key of ['free', 'pro', 'ultra']) {
        if (existing.plans?.[key]?.monthlyCalls !== undefined && existing.plans[key].monthlyTokens === undefined) {
          existing.plans[key].monthlyTokens = existing.plans[key].monthlyCalls * 1500
          needsUpdate = true
        }
      }
      // 升级 referral 中的旧字段名
      if (existing.referral?.inviterRewardCalls !== undefined && existing.referral.inviterRewardTokens === undefined) {
        existing.referral.inviterRewardTokens = existing.referral.inviterRewardCalls * 1000
        existing.referral.inviteeRewardTokens = (existing.referral.inviteeRewardCalls || 5) * 1000
        needsUpdate = true
      }
      if (needsUpdate) {
        try {
          const { _id, ...patch } = { ...existing }
