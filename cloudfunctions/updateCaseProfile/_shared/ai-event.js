...
function buildSchemaInstruction() {
  return [
    `严格返回 JSON：{"schemaVersion":${SCHEMA_VERSION},"event":{actor,interaction,commitmentStatus,commitmentType,evidenceType,scene,signals,strength},"copy":{title,summary,reason,answer,targetMind,nextStep,caution,petLine,petMood}}。`,
...
function buildSemanticInstruction(inputSubjectRole) {
  return [
    ...
    'actor 必须是最终主 interaction 的动作发出者，不是句子第一个动作的发出者。按动作发出者判断主体：“我邀请他”是 self，“他邀请我”是 target；双方均有明确且同等重要的主动作才是 both；无法判断才是 unknown。用户自己的行为、感受和猜测不是对方信号。',
    ...
    '主结果优先级：承诺未兑现 > 拒绝 > 拖延 > 兑现 > 承诺 > 回应 > 主动发起 > 普通观察 > 不清。',
    '关键正例：“我请他吃饭，他拒绝了我”→actor=target, interaction=rejected, commitmentStatus=none, commitmentType=none, evidenceType=fact, strength=strong。',
    '关键反例：“他请我吃饭，我拒绝了他”→actor=self, interaction=rejected, commitmentStatus=none, commitmentType=none, evidenceType=fact, strength=strong。',
    ...
    inputSubjectRole === 'both' ? '这是微信对话记录，请按说话人姓名或气泡方向拆分双方动作。' : '',
    '人格和建议强度只影响 copy，不得影响 event。copy 对主体、主动作和承诺的描述必须与 event 完全一致。',
    '输出 JSON 前逐项自检：actor 是否发出主 interaction；copy 主体是否与 actor 一致；broken 是否有明确先前承诺；普通拒绝是否误带 commitmentType；strength 是否符合事实强度。'
  ].filter(Boolean).join('\n')
}
...
async function analyzeTimelineEvent(params) {
  ...
  try {
    ...
    const normalized = normalizeNormalizedEventV1(parsed, { description: params.event?.description })
    if (!normalized.ok) {
