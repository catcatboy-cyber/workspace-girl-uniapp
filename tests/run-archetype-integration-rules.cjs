...
const personSelectIndex = relationPage.indexOf("phase === 'person-select'")
const stageSelectIndex = relationPage.indexOf("phase === 'stage-select'")
const screenerIndex = relationPage.indexOf("phase === 'screener'")
const quizIndex = relationPage.indexOf("phase === 'quiz'")
assert(personSelectIndex >= 0 && personSelectIndex < stageSelectIndex, 'relation protagonist selection must happen before stage selection')
assert(stageSelectIndex < screenerIndex && screenerIndex < quizIndex, 'relation flow must be protagonist -> stage -> screener -> dedicated quiz')
assert(relationPage.includes("else phase.value = selectedPersonKey.value ? 'stage-select' : 'person-select'"))
assert(relationPage.includes('startPerson(selectedPersonKey.value)'))
assert(relationPage.includes('指定匹配：'))
assert(!relationPage.includes('recommendedPersonKey'), 'screener must not choose or replace the named protagonist mid-test')
